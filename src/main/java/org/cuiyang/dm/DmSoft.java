...
public class DmSoft {
     ...
     * 加载大漠插件信息
     */
    private static void loadDmInfo() throws Exception {
        log.info("初始化大漠插件信息");
        if (!isWindows() || is64()) {
            throw new DmException("大漠插件只能在window平台使用.jdk必须是32位的.");
        }
        if (!checkDmDll()) {
            File targetFile = new File(FileUtils.getTempDirectory(), DM_DLL);
            log.info("释放资源dm.dll到指定位置({})", targetFile);
            URL resource = DmSoft.class.getClassLoader().getResource(DM_DLL);
            if (resource == null) {
                throw new DmException("释放资源dm.dll到指定文件夹失败");
            }
            FileUtils.copyURLToFile(resource, targetFile);
     ...
     * 检查大漠插件是否存在
     */
    private static boolean checkDmDll() {
        ActiveXComponent tempDm;
        try {
            tempDm = new ActiveXComponent(DM_PROGRAM_ID);
            String ver = Dispatch.call(tempDm, "Ver").getString();
            return VERSION.equals(ver);
        } catch (Exception e) {
            log.warn("创建大漠对象失败，插件没有正确获取到...");
            return false;
        }
    }
     ...
     * 注册大漠插件
     * 注册：regsvr32 dm.dll
     * 卸载：regsvr32 dm.dll /u
     */
    private static void regDmDll(File targetFile) {
        if (!targetFile.exists()) {
            log.error(targetFile + "没有找到,注册失败");
        } else {
            ...
            try {
                ...
                log.error("exec " + cmd + " failure", e);
            } finally {
                if (p != null) {
                    try {
                        p.waitFor();
                    } catch (Exception e) {
                        log.error("exec process waitFor failure", e);
                    } finally {
                        p.destroy();
                    }
                }
